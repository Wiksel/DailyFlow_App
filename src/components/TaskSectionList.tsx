...
const TaskSectionListInner = ({ tasks, categories, onPressTask, onToggleComplete, onConfirmAction, selectionMode = false, selectedIds, onToggleSelect, onOpenTaskMenu, onSelectAllSection, onQuickAdd, pinnedIds, onTogglePinned, highlightQuery }: Props, ref: React.Ref<TaskSectionListHandle>) => {
    ...
    const sections = useMemo(() => {
        ...
        const sections = order.map(({ key, title }) => ({ key, title, data: buckets[key] }));
                  ...
                  >
                    <Animated.View layout={Layout.springify()} entering={FadeInUp.delay(Math.min(300, (index || 0) * 20))} style={[styles.itemContainer, { backgroundColor: theme.colors.card, borderColor: theme.colors.border, shadowColor: Colors.shadow }]} >
                      <TaskListItem
                          ...
                          selected={selected}
                          onToggleSelect={() => onToggleSelect && onToggleSelect(item)}
                          onOpenMenu={!selectionMode ? () => onOpenTaskMenu && onOpenTaskMenu(item) : undefined}
                          pinned={isPinned}
                          onTogglePinned={() => onTogglePinned && onTogglePinned(item)}
